...
# Add sourcing line to the top of ~/.bashrc to load the dotfiles version if not already present
BASHRC="$HOME/.bashrc"
SOURCE_LINE="[ -r \"$DOTFILES/.bashrc\" ] && source \"$DOTFILES/.bashrc\""
...
if ! grep -Fxq "$SOURCE_LINE" "$BASHRC"; then
    printf "%s\n%s\n" "$SOURCE_LINE" "$(cat "$BASHRC")" > "$BASHRC"
    echo "Added dotfiles sourcing line to $BASHRC"
else
